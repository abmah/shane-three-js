...
import projectState from "./animation12.json";
...
function processFlipQueue() {
    ...
    if (!isAnimating && flipQueue.length > 0) {
        ...
        setTimeout(() => {
            ...
            counterHidden.style.transition =
...
function playHtmlAnimations() {
    ...
    heroTitleFirst.style.animation =
        "reveal 1.5s cubic-bezier(0.77, 0, 0.175, 1) 0.5s forwards";
    heroTitleSecond.style.animation =
        "reveal 1.5s cubic-bezier(0.77, 0, 0.175, 1) 0.6s forwards";
    heroSub1.style.animation =
        "reveal 1s cubic-bezier(0.77, 0, 0.175, 1) 0.9s forwards";
    heroSub2.style.animation =
        "reveal 1s cubic-bezier(0.77, 0, 0.175, 1) 1s forwards";
    heroSub3.style.animation =
        "reveal 1s cubic-bezier(0.77, 0, 0.175, 1) 1.1s forwards";
...
gui
    .addColor(transparentMaterial, "color")
    .name("transparentColor")
    .onChange(() => {
        transparentMaterial.needsUpdate = true;
    });
gui
    .add(transparentMaterial, "metalness")
    .min(0)
    .max(1)
    .step(0.001)
    .name("transparentMetalness")
    .onChange(() => {
        transparentMaterial.needsUpdate = true;
    });
gui
    .add(transparentMaterial, "roughness")
    .min(0)
    .max(1)
    .step(0.001)
    .name("transparentRoughness")
    .onChange(() => {
        transparentMaterial.needsUpdate = true;
    });

const initTexture = textureLoader.load("c12c21.jpg");
...
const tunnelTexture = textureLoader.load("1d2112dd12.jpg");
...
const emmisiveWhiteMaterial = new THREE.MeshStandardMaterial({
    color: new THREE.Color("#ffffff"),
    metalness: 0.5,
    roughness: 0,
...
gltfLoader.load("latest.glb", (gltf) => {
    ...
    const whiteBitcoinAndHandle = gltf.scene.getObjectByName(
        "white-bitcoin-handle"
    );
    ...
    const blackBitcoinAndHandle = gltf.scene.getObjectByName(
        "black-bitcoin-handle"
    );
...
gui
    .add({ count: firefliesCount }, "count", 1, 5000)
    .step(1)
    .name("firefliesCount")
    .onChange((value) => {
...
fireflies.position.set(0.23, 0, 0.5);
...
gui
    .add(firefliesMaterial.uniforms.uSpeed, "value")
    .min(0)
    .max(0.1)
    .step(0.001)
    .name("uSpeed");
...
tl.to(".to-be-animated", {
    ...
    duration: 2,
    transform:
        "perspective(500px) translate3d(0px, 0px, 0px) rotateX(0deg) rotateY(0deg) rotateZ(0deg) scale3d(1, 1, 1)",
});
...
tl.from(".first-card", {
    cssText:
        "-webkit-mask-image: linear-gradient(270deg, black 100%, transparent 200%); mask-image: linear-gradient(270deg, black -100%, transparent 0%);",
});
...
nestedTl.to(".first-card", {
    cssText:
        "-webkit-mask-image: linear-gradient(270deg, black -80%, transparent 0%); mask-image: linear-gradient(270deg, black -80%, transparent 0%);",
});
tl.from(".second-card", {
    cssText:
        "-webkit-mask-image: linear-gradient(90deg, black -100%, transparent 0%); mask-image: linear-gradient(90deg, black -100%, transparent 0%)",
});
...
nestedTl.to(
    ...
    {
        cssText:
            "-webkit-mask-image: linear-gradient(90deg, black 100%, transparent 200%); mask-image: linear-gradient(90deg, black -100%, transparent 0%);",
    },
    0.1
); // Set the position of this animation to 0, which means it starts at the same time as the previous animation
...
tl.to(".to-be-animated", {
    opacity: 0,
    transform:
        "perspective(250px) translate3d(200px,200px, 200px) rotateX(0deg) rotateY(0deg) rotateZ(0deg)",
    pointerEvents: "none",
    duration: 1.2,
});
...
textAnimation.from(".text-animation-reveal", {
    userSelect: "none",
    cssText:
        "    -webkit-mask-image: linear-gradient(90deg, black -100%, transparent 0%); mask-image: linear-gradient(90deg, black -100%, transparent 0%)",
});
textAnimation.to(".text-animation-reveal", {
    userSelect: "all",
    cssText:
        "-webkit-mask-image: linear-gradient(90deg, black 100%, transparent 200%); mask-image: linear-gradient(90deg, black -100%, transparent 0%);",
});
...
flashTl.from(".flash-card-1", {
    userSelect: "none",
    cssText:
        "    -webkit-mask-image: linear-gradient(90deg, black -100%, transparent 0%); mask-image: linear-gradient(90deg, black -100%, transparent 0%)",
});
flashTl.from(".flash-card-2", {
    userSelect: "none",
    cssText:
        "    -webkit-mask-image: linear-gradient(90deg, black -100%, transparent 0%); mask-image: linear-gradient(90deg, black -100%, transparent 0%)",
});
flashTl.from(".flash-card-3", {
    userSelect: "none",
    cssText:
        "    -webkit-mask-image: linear-gradient(90deg, black -100%, transparent 0%); mask-image: linear-gradient(90deg, black -100%, transparent 0%)",
});
